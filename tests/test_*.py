...
MIN_CPM = 0.25 / 1000  # min cost per view
MAX_CPM = 4.00 / 1000  # max cost per view
...
for rank, singer, subs in ranked_list:
    ...
    earnings_estimate = f"${round(min_earnings)} - ${round(max_earnings)}" # rounding to whole numbers
    data = {
        'fields': {
            'rank': rank,
            'artist': singer,
